...
class AdvancedChatWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.status_label = AnimatedLabel("QUANTUM NEURAL NETWORK READY")
    ...
    def paintEvent(self, event):
        ...
        # Draw particles with 3D effect (fix: use self.particles, not self.holographic_particles)
        for p in self.particles:
            color = p['color']
    ...
    def handle_error(self, error_msg):
        ...
        self.animate_quantum_error()
    
    def handle_status_update(self, status):
        """Handle QUANTUM status updates"""
        self.is_typing = False
        self.status_label.set_text_with_animation(f"QUANTUM: {status}")
    ...
    def toggle_mic(self):
        ...
        if self.backend_manager:
            if self.backend_manager.is_running:
                ...
                """)
                self.status_label.set_text_with_animation("QUANTUM NEURAL OFFLINE")
                self.voice_visualizer.set_listening(False)
            else:
                ...
                """)
                self.status_label.set_text_with_animation("QUANTUM NEURAL ACTIVE")
                self.voice_visualizer.set_listening(True)
    ...
    def paintEvent(self, event):
        ...
        for p in self.particles:
            ...
            if p['type'] == 'sphere':
                ...
                for i in range(5):
                    angle = i * 2 * math.pi / 5
                    points.append(QPoint(x + int(math.cos(angle) * size), y + int(math.sin(angle) * size)))
...
class UltraJarvisBlobWidget(QWidget):
    ...
    def add_random_speaking_effects(self):
        ...
        for particle in self.particles:
            if self.random_generator.random() < 0.3:  # 30% chance
                p['vx'] += self.random_generator.uniform(-1.0, 1.0)
                p['vy'] += self.random_generator.uniform(-1.0, 1.0)
    ...
    def update_particles(self, delta_time):
        ...
        for particle in self.particles:
            ...
            if p['movement_pattern'] == 'circular':
                # Circular movement
                angle = p['individual_phase'] + self.particle_phase * individual_speed
                radius = 30 + 20 * math.sin(p['individual_phase'] * 0.5)
                p['x'] = radius * math.cos(angle)
                p['y'] = radius * math.sin(angle)
            elif p['movement_pattern'] == 'chaotic':
                # Chaotic movement
                p['x'] += p['vx'] * delta_time * 30 * individual_speed
                p['y'] += p['vy'] * delta_time * 30 * individual_speed
                
                # Add random chaotic forces
                if self.random_generator.random() < 0.1:  # 10% chance
                    p['vx'] += self.random_generator.uniform(-0.5, 0.5)
                    p['vy'] += self.random_generator.uniform(-0.5, 0.5)
            else:
                ...
                if distance > 0:
                    ...
                    p['vx'] += dx * attraction * delta_time
                    p['vy'] += dy * attraction * delta_time
            ...
            p['life'] += delta_time * life_speed
            if p['life'] > 1.0:
                p['life'] = 0.0
                p['x'] = self.random_generator.uniform(-self.radius, self.radius)
                p['y'] = self.random_generator.uniform(-self.radius, self.radius)
                p['vx'] = self.random_generator.uniform(-0.8, 0.8)
                p['vy'] = self.random_generator.uniform(-0.8, 0.8)
                p['size'] = self.random_generator.uniform(1.5, 7)
                p['color'] = QColor(0, 255, 255, self.random_generator.randint(20, 100))
                p['movement_pattern'] = self.random_generator.choice(['circular', 'linear', 'chaotic'])
    ...
    def draw_particles(self, painter, cx, cy):
        ...
        for particle in self.particles:
            alpha = int(p['color'].alpha() * (1 - p['life']))
            if alpha > 0:
                particle_color = QColor(p['color'].red(), 
                                      p['color'].green(), 
                                      p['color'].blue(), alpha)
                
                # Draw particle with enhanced glow
                size = p['size'] * (1 + 0.5 * math.sin(p['life'] * math.pi + p['individual_phase']))
                x = cx + p['x']
                y = cy + p['y']
